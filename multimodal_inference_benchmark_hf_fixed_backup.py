...
class TimingCapture:
    """Helper class to capture TTFT (Time to First Token) by monitoring model generation"""
    ...
    def capture_first_token(self):
        if not self.first_token_captured and self.start_time is not None:
    ...
    def get_tpot(self, num_output_tokens):
        if (self.start_time and self.end_time and self.first_token_time and 
            num_output_tokens > 1):
            generation_after_first = self.end_time - self.first_token_time
            return generation_after_first / (num_output_tokens - 1)
        return None
...
def run_multimodal_inference(
    ...
    Enhanced with TTFT and TPOT timing measurements.
    """
    ...
    metrics = {
        ...
        "ttft_times": [],  # Time to First Token for each request
        "tpot_times": [],  # Time Per Output Token for each request
        "average_ttft": 0,
        "average_tpot": 0,
        ...
        try:
            ...
            for i, (image, img_path) in enumerate(zip(batch_images, valid_files)):
                try:
                    ...
                    timing_capture.start_timing()
                    
                    with torch.no_grad():
                        # Custom generation with TTFT tracking
                        generated_ids = model.generate(
                            **inputs,
                            max_new_tokens=512,  # Fixed reasonable limit
                            min_new_tokens=1,
                            temperature=temperature if temperature > 0 else None,
                            do_sample=True if temperature > 0 else False,
                            pad_token_id=processor.tokenizer.eos_token_id if hasattr(processor, 'tokenizer') else None,
                            use_cache=True,
                            # Hook to capture first token generation
                            output_scores=True,
                            return_dict_in_generate=True
                        )
                    
                    timing_capture.end_timing()
                    
                    # Extract generated ids for token counting
                    if hasattr(generated_ids, 'sequences'):
                        final_ids = generated_ids.sequences
                    else:
                        final_ids = generated_ids
                    
                    # Calculate output tokens (excluding input)
                    output_token_count = final_ids.shape[1] - input_token_count
                    
                    # Get timing metrics
                    ttft = timing_capture.get_ttft()
                    total_time = timing_capture.get_total_time()
                    tpot = timing_capture.get_tpot(output_token_count)
                    
                    # Fallback TTFT calculation if not captured precisely
                    if ttft is None and total_time is not None:
                        # Estimate TTFT as 10% of total time (rough approximation)
                        ttft = total_time * 0.1
                    
                    # Fallback TPOT calculation
                    if tpot is None and total_time is not None and ttft is not None and output_token_count > 1:
                        tpot = (total_time - ttft) / (output_token_count - 1)
                    
                    # Decode the response
                    generated_text = processor.batch_decode(final_ids, skip_special_tokens=True)[0]
                    ...
                    if ttft is not None:
                        batch_ttft_times.append(ttft)
                    if tpot is not None:
                        batch_tpot_times.append(tpot)
                    
                    print(f"    Image {i+1}: {input_token_count} input, {output_token_count} output tokens")
                    print(f"              TTFT: {ttft:.4f}s, TPOT: {tpot:.4f}s" if ttft and tpot else "")
            ...
            for i, (img_path, output_text) in enumerate(zip(valid_files, batch_outputs)):
                ...
                per_image_ttft = batch_ttft_times[i] if i < len(batch_ttft_times) else None
                per_image_tpot = batch_tpot_times[i] if i < len(batch_tpot_times) else None
            ...
            metrics["batch_times"].append({
                ...
                "ttft_times": batch_ttft_times,
                "tpot_times": batch_tpot_times
            })
            
            metrics["total_input_tokens"] += batch_input_tokens
            metrics["total_output_tokens"] += batch_output_tokens
            metrics["ttft_times"].extend(batch_ttft_times)
            metrics["tpot_times"].extend(batch_tpot_times)
            
            print(f"  Batch {batch_idx + 1} completed in {batch_inference_time:.4f}s")
            print(f"    Tokens: {batch_input_tokens} input, {batch_output_tokens} output")
            if batch_ttft_times:
                print(f"    Avg TTFT: {statistics.mean(batch_ttft_times):.4f}s")
            if batch_tpot_times:
                print(f"    Avg TPOT: {statistics.mean(batch_tpot_times):.4f}s")
    ...
    if metrics["ttft_times"]:
        metrics["average_ttft"] = statistics.mean(metrics["ttft_times"])
        metrics["median_ttft"] = statistics.median(metrics["ttft_times"])
        metrics["min_ttft"] = min(metrics["ttft_times"])
        metrics["max_ttft"] = max(metrics["ttft_times"])
        if len(metrics["ttft_times"]) > 1:
            metrics["std_ttft"] = statistics.stdev(metrics["ttft_times"])
    
    if metrics["tpot_times"]:
        metrics["average_tpot"] = statistics.mean(metrics["tpot_times"])
        metrics["median_tpot"] = statistics.median(metrics["tpot_times"])
        metrics["min_tpot"] = min(metrics["tpot_times"])
        metrics["max_tpot"] = max(metrics["tpot_times"])
        if len(metrics["tpot_times"]) > 1:
            metrics["std_tpot"] = statistics.stdev(metrics["tpot_times"])
...
def print_metrics(metrics: Dict[str, Any]):
    ...
    # New TTFT and TPOT metrics
    if metrics.get('ttft_times'):
        print("TTFT (Time to First Token) METRICS:")
        print(f"  Average TTFT: {metrics.get('average_ttft', 0):.4f} seconds")
        print(f"  Median TTFT: {metrics.get('median_ttft', 0):.4f} seconds")
        print(f"  Min TTFT: {metrics.get('min_ttft', 0):.4f} seconds")
        print(f"  Max TTFT: {metrics.get('max_ttft', 0):.4f} seconds")
        if 'std_ttft' in metrics:
            print(f"  Std Dev TTFT: {metrics['std_ttft']:.4f} seconds")
        print()
    
    if metrics.get('tpot_times'):
        print("TPOT (Time Per Output Token) METRICS:")
        print(f"  Average TPOT: {metrics.get('average_tpot', 0):.4f} seconds")
        print(f"  Median TPOT: {metrics.get('median_tpot', 0):.4f} seconds")
        print(f"  Min TPOT: {metrics.get('min_tpot', 0):.4f} seconds")
        print(f"  Max TPOT: {metrics.get('max_tpot', 0):.4f} seconds")
        if 'std_tpot' in metrics:
            print(f"  Std Dev TPOT: {metrics['std_tpot']:.4f} seconds")
        print()
    ...
    try:
        ...
        if args.show_outputs and "outputs" in metrics:
            ...
            for output in metrics["outputs"]:
                ...
                if output.get('ttft'):
                    print(f"TTFT: {output['ttft']:.4f} seconds")
                if output.get('tpot'):
                    print(f"TPOT: {output['tpot']:.4f} seconds")
